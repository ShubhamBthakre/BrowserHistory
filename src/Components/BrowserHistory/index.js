...
import './index.css'
import HistoryList from '../HistoryListItems/index'
...
class BrowserHistory extends Component {
  ...
  onDeleteHistory = uniqueId => {
    ...
    const filteredHistoryList = historyList.filter(
      eachHistory => eachHistory.id !== uniqueId,
    )

    this.setState({historyList: filteredHistoryList})
  }

  onSearchInputHistory = event => {
  ...
  render() {
    ...
    const isLengthGreaterThanZero = historyList.length > 0
        ...
        <div className="search-history-container">
          ...
          <div className="search-input-container">
            ...
            <input
              ...
              onChange={this.onSearchInputHistory}
            />
          </div>
        </div>
            ...
            <ul className="list-items-container">
              {historyList.map(eachHistoryItem => (
                <HistoryList
                  ...
                  onClickDeleteHistory={this.onDeleteHistory}
                />
              ))}
            </ul>
          ) : (
            <p className="empty-paragraph">There is no history to show</p>
          )}
        </div>
      </div>
    )
